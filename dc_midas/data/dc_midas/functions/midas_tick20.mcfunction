# Goldify
execute unless entity @s[tag=dc_isRaining] unless block ~ ~ ~ #dc_midas:water_type unless block ~ ~ ~ #drakoncore:waterloggable[waterlogged=true] run function dc_midas:goldify/lower_half
execute unless entity @s[tag=dc_isRaining] unless block ~ ~1 ~ #dc_midas:water_type unless block ~ ~1 ~ #drakoncore:waterloggable[waterlogged=true] run function dc_midas:goldify/upper_half_and_held
