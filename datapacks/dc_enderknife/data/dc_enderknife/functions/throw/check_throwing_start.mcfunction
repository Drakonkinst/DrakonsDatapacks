...
execute if entity @s[tag=dc_holdingThrowableSword] unless data entity @s SelectedItem run function dc_throwable_swords:throw/check_just_thrown

# Update throwable axe holding with a 1-tick delay
tag @s remove dc_holdingThrowableSword
execute if predicate dc_throwable_swords:holding_throwable_sword run tag @s add dc_holdingThrowableSword
