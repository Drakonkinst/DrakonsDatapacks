...
execute as @e[tag=dc_entityLook.result,limit=1,distance=..128] unless entity @s[gamemode=spectator] unless score @s dc_studyCooldown matches ..-1 unless predicate dc_spyglass_study:is_invisible run function dc_spyglass_study:study_tick 
