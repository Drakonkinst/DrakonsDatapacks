...
execute if score #IsRightClick dc_value matches 1 if predicate dc_sniffer_riding:holding_sniffer_food on vehicle if entity @s[type=sniffer,tag=dc_snifferMount] unless score @s dc_snifferBoost matches 1.. run function dc_sniffer_riding:feed/boost_sniffer
